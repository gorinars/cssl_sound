...
def prepare_split(
    ...
    for _, v in all_paths.items():
        rng.shuffle(v)
    train_paths = {
        k:v[:int(len(v)*1.0)] for k, v in all_paths.items()
    }
    valid_paths = {
    ...
    train_paths['baby crying'] += glob.glob(os.path.join(root_dir, 'vgg-sound-16k/test/baby crying/*.ogg'))
    train_paths['baby crying'] += glob.glob(os.path.join(ucry_dir, '*.wav'))
    train_paths['baby crying'] += glob.glob(os.path.join(root_dir, '../freesound_cry/*.wav'))
