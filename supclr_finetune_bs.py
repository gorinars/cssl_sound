    ...
    for task_idx in range(start_task, num_tasks):
        ...
        if hparams['ssl_checkpoints_dir'] is not None:
            ...
            hparams['modules']['prev_predictor'] = \
                hparams['prev_predictor'] = copy.deepcopy(hparams['projector'])  
            ...
            # MAJORHACK!!! LET's try not updating projector and predictor in finetuning procedure
            # for p in hparams['predictor'].parameters():
            #     p.requires_grad = False    
            # for p in hparams['projector'].parameters():
            #     p.requires_grad = False   
        ...
        if hparams["balanced_cry"]:
            from speechbrain.dataio.sampler import BalancingDataSampler
            sampler = BalancingDataSampler(
                dataset=train_data,
                key="is_cry",
            )
            hparams["train_dataloader_opts"]["sampler"] = sampler
            hparams["train_dataloader_opts"]["drop_last"] = True
